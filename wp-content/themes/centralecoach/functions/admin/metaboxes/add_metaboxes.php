...
class create_meta_boxes {
		...
		foreach($this->options as $option) {
			if (isset($option['id']) && ! empty($option['id'])) {
				if(!isset($option['only']) || $option['only'] == $_POST['post_type'] || in_array($_POST['post_type'], explode(',', $option['only']))) {
					...
					if($value != '') {
						update_post_meta(intval($post_id), $option['id'], $value);
					} else {
						delete_post_meta(intval($post_id), $option['id'], get_post_meta($post_id, $option['id'], true));
					}
				}
			}
		}
	...
	public function render() {
		...
		echo '
				<input type="hidden" name="' . esc_attr($this->config['id']) . '_noncename" id="' . esc_attr($this->config['id']) . '_noncename" value="' . esc_attr( wp_create_nonce(plugin_basename(__FILE__)) ) . '" />
			</div>';
	}
	...
	function load_template($template, $value) {
		...
		// Load template
		require_once(get_template_directory() . '/functions/admin/metaboxes/templates/' . $template . '.php');
	}

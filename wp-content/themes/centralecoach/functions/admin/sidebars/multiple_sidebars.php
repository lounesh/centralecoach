...
class sidebar_generator {
	...
	static function init() {
		...
		if (is_array($sidebars)) {
			foreach ($sidebars as $sidebar) {
				$sidebar_class = esc_html( sanitize_title( sidebar_generator::name_to_class($sidebar['name']) ) );
				...
				if ($sidebar['type'] == 'normal' || $sidebar['type'] == 'normal_gray' || $sidebar['type'] == 'normal_blue') {
					register_sidebar(array(
						...
						'after_title' => '</h3><div class="wl-sidebar-content">',
					));
				}
			}
		}
	...
	function add_sidebar() {
		...
		$js = "
			...
			//var textNode = document.createTextNode('[<a href=\'javascript:void(0);\'>Remove</a>]');
			//cellLeft.appendChild(textNode)
	...
	static function admin_page() {
		...
		<div class="wrap">
				...
				if (is_array($sidebars) && !empty($sidebars)) {
					...
					foreach ($sidebars as $sidebar) {
						...
						<tr class="<?php echo $alt ?>">
							...
							<td><?php echo esc_html($sidebar['type']); ?></td>
							<td><?php echo sidebar_generator::name_to_class($sidebar['name']); ?></td>
							<td><a href="javascript:void(0);" class="whitelab-remove-sidebar" title="Remove this sidebar">remove</a></td>
						</tr>
						<?php
						$cnt++;
					}
			...
			<div class="add_sidebar">
				<button href="javascript:void(0);" title="Add a sidebar" class="button-primary add-sidebar-button">+ Add New Sidebar</button>
			</div>
		</div>
		<?php
	}
					...
					<div class="sbg_container">
						...
						<ul>
							...
							for ($i = 0; $i < 5; $i++) {
								?>
								<li>
									<select name="sidebar_generator[<?php echo $i ?>]" class="hidden">
										<option value="0"<?php

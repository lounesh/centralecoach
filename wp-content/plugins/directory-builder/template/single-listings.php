	...
	if ( !is_user_logged_in() ) {
		...
		if ( $claim_info == '' && intval($claimed_listings['0']->claimed_listings) < $user_max_claims ) { // Check if listing is not claimed and he hasn't claimed too much listings
			...
			if ( !empty($package_list) ) {
				$output = '<div class="db-pricing-main-wrapper default">';
...
<div class="db-single-listing-wrapper default">
		...
		<div class="db-single-listing-left">
			...
			if ( isset($all_fields['amenities']) && $all_fields['amenities']['value'] != '' && !empty($all_fields['amenities']['value']) ) {
				...
				foreach ($select_data as $select_value) {
					...
					$select_options[$select_option_data['0']] = $select_option_data['1'];
				}
				...
				foreach ($amenities_values as $amenitie_value) { ?>
					<span class="db-amenitie-item">
						<?php
						echo '<a href="'.add_query_arg( 'amenities', $amenitie_value, get_permalink($main_settings['search_page_id']) ).'">'.esc_html($select_options[$amenitie_value]).'</a>'; ?>
					</span><br>
					<?php
		...
		<div class="db-single-listing-right">
						...
						if ( $main_settings['contact_form_status'] && $show_contact_form && $listing_email != '' ) {
							...
							<div id="db-contact-listing-dialog" class="db-dialog-animation">
								<div class="db-contact-form">
									<span class="db-single-listing-side-bottom">'. esc_html__('Please don\'t hesitate to send us a message!', 'whitelab').'</span>
									...
									<div class="db-contact-listing-msg">
										...
										<button class="db-contact-author" data-author="'.esc_attr($listing_email).'">Send</button>
									</div>
								</div>
			...
			<?php } ?>
				<?php
					...
					if ( $claim_info == '' ) {
						?>
						<span class="db-single-listing-side-bottom text-align-left"><?php esc_html_e('Claim your business listing on Whitelab.', 'whitelab'); ?> <a href="<?php echo esc_url(get_permalink()); ?>?db-claim"><?php esc_html_e('Claim my business.', 'whitelab'); ?></a></span>
						<?php
					} else if ( is_user_logged_in() && !empty( $claim_matches['0'] ) && get_current_user_id() === intval($claim_data['0']) ) {
						?>
						<span class="db-single-listing-side-bottom text-align-left"><?php esc_html_e('You have claimed this listing!', 'whitelab'); ?></span>
						<?php
					} else if ( is_user_logged_in() && strpos($claim_info, get_current_user_id().':waiting') !== false ) {
						?>
						<span class="db-single-listing-side-bottom text-align-left"><?php esc_html_e('Your claim is waiting for approval!', 'whitelab'); ?></span>
						<?php
					}
				?>
			</div> <!-- end of db-single-listing-side-wrapper -->
